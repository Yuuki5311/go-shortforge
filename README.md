...
常见问题
- 无法连接数据库：
  - 确认 MySQL/Redis 已启动（docker compose ps）
  - DSN/端口与 config.toml 一致（如密码变更需同步）
- 端口冲突：
  - 修改 docker-compose.yml 的端口映射或 config.toml 的端口配置
- Docker 拉取镜像超时（TLS handshake timeout）：
  - 重启 Docker Desktop 与 WSL；在 Settings → Resources → Proxies 配置代理
  - 在 Settings → Docker Engine 添加 registry-mirrors 与 dns 后重试
- 表迁移报错（long_url 索引）：
  - long_url 使用 varchar(512) + 唯一索引，避免 MySQL 前缀与长度限制（见 internal/model/model.go）
- 调试日志：
  - 设置环境变量 EGO_DEBUG=true，查看组件请求详细信息

安全与生产建议
- 配置中的明文密码仅用于本地演示，请在生产使用安全的凭据管理方案
- 可加入限流/熔断/防缓存击穿与穿透等保护
- 可接入 Prometheus 指标与 OpenTelemetry 链路追踪以提升可观测性
